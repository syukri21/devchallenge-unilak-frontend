...
class CalendarEvent extends React.Component {
  ...
  componentDidMount() {
    ...
    if (!data.loading) {
      data.events.map((e, i) =>
        arrayDate.push(moment(e.date).format("YYYY-MM-DD"))
      );
      this.setState({
        data: arrayDate
      });
  ...
  cekTileContent(date, view) {
    if (this.state.data.length !== 0) {
      return (
        view === "month" &&
        _.includes(this.state.data, moment(date).format("YYYY-MM-DD"))
      );
    }
  }
    ...
    return (
      <Calendar
        ...
        tileContent={({ date, view }) =>
          this.cekTileContent(date, view) ? <div style={styled}> </div> : null
        }
      />
    );
  }
